...
public class PumpkinControl : IChracterControl
{
    ...
    private void AutoControl()
    {
        ...
        {
            // 자유영역이 없고, 도착지에 있으면
            if (fFreeMoveDist == 0 && vDestPos == this.transform.position)
            {
                this.transform.rotation = qDestRot;
            }
            // 자유영역을 넘어가면 원위치로 돌아가기
            else if ((vDestPos - this.transform.position).magnitude > fFreeMoveDist)
            {
